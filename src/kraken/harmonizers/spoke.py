...
class SpokeHarmonizer:
    ...
    def harmonize(
        ...
        node_count = 0
        edge_count = 0
        with jsonlines.open(nodes_output, "w") as nodes_writer, jsonlines.open(edges_output, "w") as edges_writer:
    ...
    def _harmonize_node(self, node_item: dict) -> Optional[dict]:
        ...
        if primary_source == "Complex Portal" and node_type == "Complex":
            original_identifier = properties[
                "complex_portal"
            ]  # The 'identifier' for these is meaningless; true standard id is tucked away here

        # Normalize the identifier
        normalized_id, iri = self.id_norm.normalize_spoke_identifier(
    ...
    def _map_spoke_edge_type_to_biolink(
        ...
        type_map = {
            ...
            },  # SPOKE only uses for SocioeconomicExposure-->GeographicLocation edges TODO: would occurs_in be better?
            ...
            "DOWNREGULATES": {
                predicate: "regulates",
                OBJ_DIRECTION_QUALIFIER: "downregulated",
            },  # Anatomy-->Gene, Compound-->Gene, Gene-->Gene, Variant-->Gene
            ...
            "PARTICIPATES": {
                ...
                flip: True,
            },  # Gene-->BioProcess, Gene-->CellComponent, etc. TODO: these violate domain/range!
            "BELONGS": {
                predicate: "related_to"
            },  # Variant-->Gene, Haplotype-->Gene, Variant-->Haplotype  TODO: need to improve this.. has_part?
            ...
            "PREVALENCE": {
                predicate: "associated_with"
            },  # Disease-->GeographicLocation... TODO: would occurs_in be better?
            ...
            "MEASURESIN": {
                predicate: "related_to"
            },  # ClinicalLab-->Anatomy  TODO: Flag these edges as being measured_in?
            ...
            "MENTIONED_CLINICAL_TRIALS_FOR": {
                predicate: "mentioned_in_clinical_trials_for"
            },  # TODO: ask Gwenlyn what she does with these mentions..
            "IN_CLINICAL_TRIALS_FOR": {predicate: "in_clinical_trials_for"},
            "CATALYZES": {predicate: "catalyzes"},  # EC-->Reaction
            "MARKER_NEG": {
                predicate: "exacerbates_condition"
            },  # Gene-->Disease. technically gene is not in the domain, but hard to find a better one..
            "MARKER_POS": {
                predicate: "ameliorates_condition"
            },  # Gene-->Disease. technically gene is not in the domain, but hard to find a better one..
        ...
        return (
            f"{BIOLINK_PREFIX}:{type_mapping[predicate]}",
            subject_id,
            object_id,
            type_mapping.get(QUALIFIED_PREDICATE),
            type_mapping.get(OBJ_DIRECTION_QUALIFIER),
            type_mapping.get(OBJ_ASPECT_QUALIFIER),
        )
    ...
    def _normalize_source(self, spoke_source: str, spoke_edge: dict) -> str:
        ...
        if spoke_source_cleaned in mappings:
            ...
            logging.error(
                f"Encountered an unmapped edge source: {spoke_source} ({spoke_source_cleaned}). Edge: {spoke_edge}"
            )
            sys.exit(1)
