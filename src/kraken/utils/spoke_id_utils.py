...
from kraken.utils.constants import KNOWN_INVALID
...
class SpokeIDNormalizer:
    ...
    @staticmethod
    def _load_curie_construction_map() -> dict[tuple[str, str], str | list[str]]:
    ...
    def normalize_spoke_identifier(
        self, node_type: str, source: str, identifier: Any, properties: dict
    ) -> tuple[str, str]:
        """
                ...
                if isinstance(id_prop_value, list):
                    for equiv_id in id_prop_value:
                        if equiv_id and str(equiv_id).strip() and equiv_id.lower() not in none_strings:
                            equiv_curie, iri = self.normalize_spoke_identifier(
                                node_type, id_prop_name, str(equiv_id), properties
                            )
                            if equiv_curie and equiv_curie != KNOWN_INVALID:
                                equivalent_ids.add(equiv_curie)
                # Handle string values
                elif (
                    isinstance(id_prop_value, str)
                    and id_prop_value.strip()
                    and id_prop_value.lower() not in none_strings
                ):
                    equiv_curie, iri = self.normalize_spoke_identifier(
                        node_type, id_prop_name, id_prop_value, properties
                    )
                    if equiv_curie and equiv_curie != KNOWN_INVALID:
                        equivalent_ids.add(equiv_curie)
    ...
    def is_trusted_id_property(property_name: str) -> bool:
        ...
        if prop_name_lower in exact_fields or (
            first_word in equiv_id_sources and ("id" in prop_name_lower or "_list" in prop_name_lower)
        ):
            return True
        else:
            return False
