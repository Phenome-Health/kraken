...
class NodeModel:
    ...
    # Single-valued, matching Biolink's taxon slot: entity resolution enforces one taxon per merged entity.
    # A source naming several taxa for one node has conflated distinct entities, so that node is left without
    # a taxon rather than one being picked arbitrarily (see create_node).
    taxon = PropertyDef("taxon", str)
...
class EdgeModel:
    ...
    supporting_sources = PropertyDef("supporting_data_sources", list, inner_type=str)
