...
def harmonize_clingen(
    ...
    for record in data:
        ...
        if context_data and context_data['status'].get('stg2', '').lower() != 'incomplete':
            ...
            assertions_by_gene, outcome_intervention_pairs = scrape_actionability_scores(source_iri, doc_id)
            ...
            for gene_info in genes:
                ...
                # Create gene node
                gene_node = _process_gene(gene_symbol, gene_omim, normalizer)
                ...
                for disease_info in diseases:
                    ...
                    disease_node = _process_disease(disease_info, gene_omim, normalizer)
                    if disease_node:
...
def _create_gene_disease_edge(
    ...
    matching_assertion = _find_matching_assertion(gene_symbol, disease_local_ids, assertions_by_gene)
    ...
    if matching_assertion:
        attributes[CLINGEN_CURIE]['gene_condition_assertion'] = matching_assertion

    # Add outcome-intervention pairs (same for all edges from this record)
    if outcome_intervention_pairs:
        attributes[CLINGEN_CURIE]['outcome_intervention_pairs'] = outcome_intervention_pairs
...
def scrape_actionability_scores(
        url: str,
        doc_id: str,
        debug: bool = False
    ...
    try:
        ...
        logging.info(f"Assertions by gene: {assertions_by_gene}")
        logging.info(f"Outcome intervention pairs: {outcome_intervention_pairs}")
