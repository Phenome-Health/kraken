...
class IdentifierNorm:
    def __init__(self, biolink_version: str):
        self.validator_prop = 'validator'
        self.cleaner_prop = 'cleaner'
        self.normalized_prefixes_to_iris, self.prefix_lowercase_map = self.load_prefix_info(biolink_version)
        self.validator_map = self.load_validator_map()

    @staticmethod
    def load_prefix_info(biolink_version: str) -> Tuple[Dict[str, str], Dict[str, str]]:
        ...
        prefix_to_iri_map = load_biolink_file(url, biolink_version)
    ...
    def load_validator_map(self) -> Dict[str, Dict[str, Callable]]:
