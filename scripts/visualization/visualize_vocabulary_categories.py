...
def filter_data_for_visualization(df: pd.DataFrame, min_count: int = 50) -> pd.DataFrame:
    ...
    filtered_df = df[(df["vocabulary"].isin(valid_vocabs)) & (df["category"].isin(valid_categories))].copy()
...
def create_heatmap(df: pd.DataFrame, output_dir: Path):
    ...
    sns.heatmap(heatmap_data, annot=True, fmt="g", cmap="YlOrRd", cbar_kws={"label": "Node Count"}, square=False)
...
def categorize_node_theme(category: str) -> str:
    ...
    # Disease/Phenotype entities (reds/pinks)
        ...
        term in clean_cat for term in ["Disease", "Phenotypic", "Symptom", "ClinicalFinding", "BehavioralFeature"]
    ):
        return "Disease/Phenotype"

    # Anatomy/Biology entities (purples)
    elif any(term in clean_cat for term in ["Anatomical", "Cell", "Tissue", "Organ", "OrganismTaxon", "Cellular"]):
        return "Anatomy/Biology"

    # Pathway/Process entities (oranges)
    elif any(term in clean_cat for term in ["Pathway", "Process", "Activity", "Function", "Event", "BiologicalEntity"]):
...
def create_bubble_chart(df: pd.DataFrame, output_dir: Path):
    ...
    for theme in theme_totals.index:
        ...
        fig = px.scatter(
            ...
            size_max=50,
            category_orders={"vocabulary": vocab_order, "category": category_order},
        )  # Custom ordering
        ...
        fig.update_coloraxes(colorbar_title="Node Count", colorbar=dict(tickvals=tick_values_log, ticktext=tick_labels))
...
def create_network_graph(df: pd.DataFrame, output_dir: Path):
    ...
    fig.update_layout(
        title="Vocabulary-Category Network Graph (Filtered: ≥1000 counts)<br><sub>"
        "Edge thickness represents node count</sub>",
        showlegend=True,
