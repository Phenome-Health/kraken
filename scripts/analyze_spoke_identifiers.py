    ...
    with jsonlines.open(input_file, 'r') as reader:
        for line_num, record in enumerate(reader, 1):
            ...
            if record.get('type') == 'relationship':
                logging.info(f"Reached the first edge. Stopping.")
                break
...
def print_summary(examples_file: Path):
    ...
    for node_type in sorted(examples.keys()):
        ...
        for source, example in examples[node_type].items():
            ...
            properties = ', '.join(example.get('property_names', []))[:60] + '...' if len(', '.join(example.get('property_names', []))) > 60 else ', '.join(example.get('property_names', []))
            print(f"  {source:15} | {identifier:20} | {name:43} | {properties}")


def main():
    parser = argparse.ArgumentParser(
        description='Analyze SPOKE identifier patterns by type and source'
    )
    parser.add_argument('input_file', type=Path, 
                        help='SPOKE JSONL file to analyze')
